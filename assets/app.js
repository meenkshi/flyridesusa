...
var product = {
  init: function(){
    ...
    //Initiates Hover to Zoom, disabled on all touch devices
    if (slideshowAnimation == 'zoom') {
      $('.image__container img', $productSection)
        .wrap('<div></div>')
        .parent()
        .append('<div class="transition-helper"></div>')
        .zoom({
          magnify: 1,
          touch: false
        });
    ...
    var slider = $('.product_slider').flexslider({
      ...
      before: function(slider) {
      	if (started === true && slideshowAnimation === 'slide') {
          // for the initial transition
          next_height = slider.find('.slides > li.flex-active-slide').height();
          slider.find('ul.slides').css({'max-height': next_height});
          // for the rest
	      next_height = slider.find('.slides > li.flex-active-slide + li').height();
		  slider.find('ul.slides').css({'max-height' : next_height});
      	}
      }
    });
  ...
  loadProductRecommendations : function() {
        ...
        var recomTitle = $('.product-recommendations').data('title');
...
var productFormListeners = {
  init: function(){
    ...
    $(document).on("click", ".plus", function(e) {
      ...
      var dataId = parseInt($(this).prev("input").attr('data-id'));      
      var reserveId = "32650248618030";
      if(dataId != reserveId){
      quantity += 1;
...
var ajaxCart = {
  init: function(){
    $(document).on('click', '.ajax-submit', function(e) {
      ...
      var rebateText = $('.rebateValueText').val();
      var rebateAmt = $('.rebateAmtText').val();
      
      var $addToCartForm = $(this).closest('form');
      var $addToCartBtn = $addToCartForm.find('.add_to_cart');
      var productForm_data = $addToCartForm.serialize() + "&note=" + "";
        var vid = "32650248618030";
     
    
    
      if ($(this).hasClass("preorder-button")) {
          //32650248618030
          //4645930237998
          
          var toreserveproid = $(this).attr( "data-proid" );
          var toreservevid = $(this).attr( "data-vid" );
          var newCartNote = '';
          if(toreserveproid != '' && toreservevid != '' ){
             newCartNote = "Product: "+ toreserveproid + "\n"+"Variant: "+ toreservevid;
          }        

         if(rebateText == 'yes'){
              if(rebateAmt ==200){
               var rebateProductId = 32214585442350; 
              }else{
                var rebateProductId =  32214585507886;
              }
                 var productForm_data = {
                      items: [
                        {
                          id: 32650248618030,
                          quantity: 1,
                 ...
                 productForm_data = { id:vid,quantity:1, note: newCartNote };
              }
      ...
      url: '/cart/add.js',
      dataType: 'json', 
      data: productForm_data, 
      success: function(data){
        console.log("reserve prduct added");
        $.ajax({
            url: '/cart.js',
            dataType: "json",
            cache: false,
    ...
    // console.log(toreservevid);
    // console.log(newCartNote);
       
    else {      
     
     //ajaxUpdateCart( vid, 0, $('#cart input.quantity').parents('#cart') ); 
      var data = { updates: {} };
      data.updates[vid] = 0;
      ...
      $.ajax({
        ...
        success: function(itemData) {
          ...
          $.ajax({
            ...
            success: function (data) {
             // refreshCart(data);
              console.log("reserve product removed");
               $.ajax({
            url: '/cart.js',
            dataType: "json",
            cache: false,
           ...
           //REBATE PRODUCT ADD ACCORDING TO CONDITION START
            var rebateVal = $( ".rebateval" ).val();
            var rebateamt = $( ".rebateamt" ).val();
            if(rebateamt != null && rebateamt == 400){
               var productVariantId = 32214585507886;           
              }
              if(rebateamt != null && rebateamt == 200){
                var productVariantId = 32214585442350;
              }
            if( rebateVal == 'yes' && rebateVal != null ){
              setTimeout(function(){
                $.ajax({
                  type: 'POST',
                  url: '/cart/add.js',
          ...
          //REBATE PRODUCT ADD ACCORDING TO CONDITION END
        },
    ...
    if (cart.item_count == 0) {
      ...
      $.each(cart.items, function(index, item) {
        ...
        for (i=0; i < itemDiscounts.length; i++) {
          ...
          discountMessage = '<span class="notification-discount"><strong>Discount:</strong> ' + title + ' (-<span class="money">' + amount + '</span>)</span>';
        }

        if(item.price > item.final_price) {
          var itemPrice = '<span class="money">' + Shopify.formatMoney(item.final_price, $('body').data('money-format')) + '</span>' + ' <span class="money was_price">' + Shopify.formatMoney(item.price, $('body').data('money-format')) + '</span>';
          var linePrice = '<span class="money">' + Shopify.formatMoney(item.final_line_price, $('body').data('money-format')) + '</span>' + ' <span class="money was_price">' + Shopify.formatMoney(item.original_line_price, $('body').data('money-format')) + '</span>';
        } else {
        ...
        cart_items_html += '<div class="price"><strong>Item total: </strong>' + linePrice + '</div></div></a></li>';
      ...
      if (cart.cart_level_discount_applications.length > 0) {
        cart_items_html += '<li class="mm-label">' + '<a href="/cart">' + '<strong>Discount</strong>' + '</a>' + cartDiscountMessage + '</li>'
      }
...
$(function(){
  ...
  $('#shopify-section-16149582883adf8269 .logo-bar').slick({
    slidesToShow: 5,
    responsive: [
      {
        breakpoint: 768,
